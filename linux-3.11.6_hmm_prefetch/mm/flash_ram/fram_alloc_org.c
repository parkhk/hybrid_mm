...
unsigned long check_fram_call[8];
EXPORT_SYMBOL(check_fram_call);
static void run_fram_io(struct softirq_action *h)
{
	int this_cpu = smp_processor_id();
	udelay(800);
	check_fram_call[this_cpu]++;

}
__init void init_fram_io_d(void)
{
	open_softirq(FRAM_SOFTIRQ, run_fram_io);
}
